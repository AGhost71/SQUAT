...
__global__ void SigmoidFocalLossForward(const int nthreads, 
  ...
  CUDA_1D_KERNEL_LOOP(i, nthreads) {
    ...
    // Decide if it is a positive or negative case. 
    T c1 = (t == (d + 1)); 
    T c2 = (t >= 0 && t != (d + 1));
    ...
    // p = sigmoid(logits)
    T p = 1.0 / (1.0 + expf(-logits[i]));

    // Compute focal loss components
    T term1 = powf((1.0 - p), gamma) * logf(max(p, FLT_MIN));
    T term2 = powf(p, gamma) * (-1.0 * logits[i] * (logits[i] >= 0) - logf(1.0 + expf(logits[i] - 2.0 * logits[i] * (logits[i] >= 0))));
...
__global__ void SigmoidFocalLossBackward(const int nthreads,
  ...
  CUDA_1D_KERNEL_LOOP(i, nthreads) {
    ...
    T term1 = powf((1.0 - p), gamma) * (1.0 - p - (p * gamma * logf(max(p, FLT_MIN))));
    T term2 = powf(p, gamma) * ((-1.0 * logits[i] * (logits[i] >= 0) - logf(1.0 + expf(logits[i] - 2.0 * logits[i] * (logits[i] >= 0)))) * (1.0 - p) * gamma - p);
