...
__global__ void nms_kernel(const int n_boxes, const float nms_overlap_thresh,
  ...
  if (threadIdx.x < row_size) {
    ...
    const int col_blocks = (n_boxes + threadsPerBlock - 1) / threadsPerBlock;  // Replaced THCCeilDiv
...
at::Tensor nms_cuda(const at::Tensor boxes, float nms_overlap_thresh) {
  ...
  const int col_blocks = (boxes_num + threadsPerBlock - 1) / threadsPerBlock;  // Replaced THCCeilDiv

  scalar_t* boxes_dev = boxes_sorted.data_ptr<scalar_t>();

  // Allocate memory with modern APIs
  unsigned long long* mask_dev = nullptr;
  mask_dev = static_cast<unsigned long long*>(c10::cuda::CUDACachingAllocator::raw_alloc(
